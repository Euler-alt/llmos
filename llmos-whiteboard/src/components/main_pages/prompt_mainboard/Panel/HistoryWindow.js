...
const HistoryItemContent = ({ item, darkMode }) => {
  ...
  return (
    <div className={`p-4 border-t ${darkMode ? 'border-gray-600' : 'border-gray-200'}`}>
      ...
      <div 
        className={`
          max-h-96 custom-scrollbar relative group
      ...
      >
        {!isScrollActive && (
          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-5 flex items-center justify-center transition-all duration-300 z-10">
            <div className={`
              px-4 py-2 rounded-full text-[10px] font-medium shadow-sm opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300
        ...
        {activeTab === 'raw' && (
          <div className={`p-3 rounded text-sm font-mono whitespace-pre-wrap break-all ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
