...
const HeapWindow = ({ data, onUpdate, darkMode }) => {
  ...
  return (
    <div className={`
      ...
      ${isUpdated ? (darkMode ? 'ring-2 ring-green-500' : 'ring-2 ring-green-400') : ''}
      shadow-lg border ${darkMode ? 'border-gray-700' : 'border-blue-200'}
    `}>
      {/* 主头部 */}
      <div className={`
        p-4 flex justify-between items-center
        ${darkMode ? 'bg-green-900 hover:bg-green-800' : 'bg-green-600 hover:bg-green-500'} 
      ...
      `}>
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
          ...
          <div>
            <h4 className="font-semibold text-lg">堆窗口 (Heap)</h4>
            <p className="text-green-100 text-xs opacity-80">持久化存储区域</p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="space-y-4 p-4">
          ...
          <div className={`
            ...
            ${darkMode ? 'border-gray-600 bg-gray-750' : 'border-green-200 bg-green-100'}
          `}>
            <div className={`
              p-3 flex justify-between items-center cursor-pointer
              ${darkMode ? 'bg-green-800 hover:bg-green-700' : 'bg-green-500 hover:bg-green-400'}
              text-white transition-colors duration-200
                ...
                <textarea
                  ...
                  className={`
                    ...
                    ${darkMode 
                      ...
                      : 'bg-white text-gray-800 border-gray-300 focus:ring-green-400'
                    }
                  `}
                  value={meta || ''}
